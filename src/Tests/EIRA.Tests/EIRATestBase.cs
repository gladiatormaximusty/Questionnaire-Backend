...
namespace EIRA.Tests
{
    public abstract class EIRATestBase : AbpIntegratedTestBase<EIRATestModule>
    {
        ...
        protected void UsingDbContext(Action<EIRADbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EIRADbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<EIRADbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<EIRADbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EIRADbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EIRADbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EIRADbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EIRADbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<EIRADbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<EIRADbContext, Task<T>> func)
        {
            T result;
